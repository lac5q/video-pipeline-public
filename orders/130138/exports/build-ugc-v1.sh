#!/bin/bash
# TY-130138 UGC v1 — Printful mockups + Gemini lifestyle staging
# Couple portrait, landscape illustration, Christmas gift theme
# 12 products with labels, reaction audio mixed with music, blurred bg for all
# Structure: Hook(2s) → Reaction(4s) → Photos(3s) → Illustration(1.5s) → 12 Products(12s) → Logo(2s) = ~24.5s
set -euo pipefail

WORKSPACE="/Users/lcalderon/clawd/agents/gwen/workspace/turnedyellow-video-ugc-130138"
MOCKUPS="${WORKSPACE}/mockups"
PHOTOS="${WORKSPACE}/photos"
ILLUSTRATION="${WORKSPACE}/illustration.jpg"
REACTION="${WORKSPACE}/130138.mov"
...
# Photos (all 3)
for p in photo1 photo2 photo3; do
    echo "  Preparing ${p}..."
...
magick -size ${W}x${H} xc:black \
    -font "/System/Library/Fonts/HelveticaNeue.ttc" \
    -pointsize 58 -fill white -gravity center \
    -annotate +0+0 "you'll find no other gift" \
    "${TMP}/hook_beat1.png"

magick -size ${W}x${H} xc:black \
    -font "/System/Library/Fonts/HelveticaNeue.ttc" \
    -pointsize 58 -fill white -gravity center \
    -annotate +0-50 "you'll find no other gift" \
    -pointsize 50 -fill "#FFD700" \
    -gravity center -annotate +0+50 "that makes them\nlaugh like this" \
    "${TMP}/hook_beat2.png"

# === Reaction video processing ===
echo "=== Processing reaction video ==="

# Reaction from 0:02 to 0:06 (4 seconds)
ffmpeg -y -ss 0 -i "${REACTION}" -t 6 \
...
# 2. Reaction video (6s — starts 2s before the reaction at 0:02)
echo "  Reaction video (6s, video only)..."
...
add_music_with_reaction "${MUSIC_CANDY}" "${EXPORTS}/TY-130138-ugc-v1-candyland.mp4"

echo "=== DONE ==="
ls -lh "${EXPORTS}/TY-130138-ugc-v1-"*.mp4
